#📄 Documentation: Luxury Housing Sales Analysis – Bengaluru
------------------------------------------------------------------------------------------------------------------
##Project Summary
===============
...
Handling Nulls: I removed rows with missing price data to ensure the financial analysis remained accurate.
Feature Engineering: Created new metrics like Price_per_Sqft and a Booking_Flag for better visualization.
Normalization: Cleaned builder and location names to remove duplicates caused by typos or extra spaces.
...
This ensured data integrity and allowed for structured querying.
...
Interactivity: Added slicers for Builders and Markets so users can filter the whole report.
Custom Logic: Wrote DAX measures to calculate the "Average Market Amenity Score."
Spatial Analysis: Integrated a Map visual to show the geographic density of luxury projects.
...
##SQL Data Validation
=======================
To ensure the data was loaded correctly into the SQL database, I executed the following validation queries. These results confirmed the integrity of the data before building the dashboard:
...
Purpose: To ensure no data was lost during the migration from CSV to SQL.
...
Purpose: To validate that the financial columns are numeric and aggregations are working.

-**3. Null Value Audit**

Query: SELECT COUNT(*) FROM luxury_housing WHERE Ticket_Price_Cr IS NULL;
Purpose: To confirm that the Python cleaning step (dropping nulls) was successful.
...
=========================
After analyzing the data, here are my top 3 findings:
Market Hotspots: Luxury housing is highly concentrated in the East (Whitefield) and South (Sarjapur) corridors, likely due to tech park proximity.
Product Demand: There is a massive shift toward 3BHK and 4BHK units, showing that luxury buyers in Bengaluru now prioritize space over smaller, central units.
The "Amenity Gap": The average amenity score is 7.50, indicating that high-end facilities (gyms, infinity pools, etc.) are now a standard expectation rather than a "bonus" in this segment.

-----------------------------------------------------------------------------------------------
##Repository Structure
========================
clean_and_load.py: The Python script for cleaning and SQL insertion.
real_estate.db: The SQL database containing the cleaned data.
Luxury_Housing_Analysis.pbix: The final Power BI dashboard.
...
Connecting Power BI to SQL databases using Python.
Writing DAX for business KPIs.
Translating complex data into simple, visual business stories.